...
export default function Home() {
    ...
    async function checkUserSession() {
      try {
        ...
        if (session?.user) {
          ...
          // Wir fragen das echte Schema ab: avatar_url enthält den Key, display_name den Usernamen!
  ...
  const handleRegister = async () => {
    ...
    try {
      // 1. Anonyme Session bei Supabase triggern -> Triggert automatisch euren DB-Trigger!
      const { data: authData, error: authError } = await supabase.auth.signInAnonymously();
      if (authError) throw authError;
      if (!authData.user) throw new Error('Keine valide User-ID erhalten');

      const anonymousUserId = authData.user.id;
      
      // 2. Wir generieren deinen unverkennbaren Hyphen-Schlüssel direkt im Frontend
      ...
      // 3. Da der DB-Trigger die ID bereits angelegt hat, aktualisieren wir den Eintrag direkt!
      ...
      alert(`Hoppla! Da gab es ein Problem: ${error.message || 'Bitte noch einmal versuchen.'}`);
    } finally {
      setLoading(false);
    }
  };
