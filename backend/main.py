...
def analyze_match(fixture_id: int, team_home_id: int, team_away_id: int):
    ...
    score = 50 + (wins_away - wins_home) * 5 + (h2h_away_wins - h2h_home_wins) * 4 - len(injuries_away) * 5 + len(injuries_home) * 3
...
async def get_matches():
    try:
        ...
        for match in matches:
            analysis_response = supabase.table("match_analysis").select("*").eq("api_fixture_id", match["api_fixture_id"]).execute()
...
def get_user_tip_for_fixture(username: str, fixture_id: int):
    try:
        ...
        user_id = user_check.data[0]['id']
        result = supabase.table('user_tips').select("*").eq('user_id', user_id).eq('api_fixture_id', fixture_id).execute()
        if not result.data:
...
def save_match_result(result: MatchResultInput):
    try:
        result_data = {"api_fixture_id": result.api_fixture_id, "home_score": result.home_score, "away_score": result.away_score, "status": result.status}
